...
for row in range(<開始行>, <終了行>+1):
    for col in range(1, 22):  # A〜U列（5曜日×4列＋A列）
...
**シート構造の目安**：
- 各月シートに週ブロックが縦に並ぶ。1週ブロック = 約30行（日付行・行事/予定行・1限〜6限×各3行・準備物・下校時刻 等）
- 各曜日は4列を使用：月曜=B/C/D/E、火曜=F/G/H/I、水曜=J/K/L/M、木曜=N/O/P/Q、金曜=R/S/T/U
  - 日付セルの位置：月曜=B列、火曜=F列、水曜=J列、木曜=N列、金曜=R列
  - 教科名は各曜日の1列目（B, F, J, N, R）、単元名は2列目（C, G, K, O, S）
- 5月の週開始行参考：4/27週=44行付近、5/4週=53行付近、5/11週=61行、5/18週=91行
- データが見つからない週は該当行を `range(行-3, 行+30)` 程度の範囲で全セル走査して位置を確認すること
